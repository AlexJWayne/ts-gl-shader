...
> = RemoveComments<TSrc> extends `${string}${TQualifier}${infer TDeclaration};${infer TRest}`
  ? ParseTokens<TDeclaration> extends
      | [GlslPrecision, infer TVarType extends GlslVarType, infer TIdenitifier extends string]
      | [infer TVarType extends GlslVarType, infer TIdenitifier extends string]
    ? {
        [name in TIdenitifier]: { type: TVarType }
      } & GlslVarsInfo<TRest, TQualifier>
    : {}
  : {}

type A = GlslVarsInfo<'uniform mediump float uAdf;', 'uniform'>
